...
class Settings:
    ...
    # プレイ表示をこのフレーム数連続で「同じカード」に一致したら記録する。単発の偶然一致
    # (何も出ていないのに背景の帯OCRがたまたまカード名に一致する誤検出)を弾くために必要。
    # 本物のプレイ表示は1〜2秒出続けるので連続一致する。取りこぼしが多い環境では1に下げる。
    reveal_confirm_frames: int = 2
