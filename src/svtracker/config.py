...
class Settings:
    ...
    # プレイ表示をこのフレーム数連続で確認したら記録。名前OCR経路は帯スライドOCRの
    # 処理時間で実効フレーム間隔が延び、表示中に2回試行できない場合があるため既定1
    # (実対戦動画で、本物の一致度0.84-0.93 vs 非表示時0.3以下と分離が明確なことを確認済み。
    #  誤検出が出る環境では2に上げる)。
    reveal_confirm_frames: int = 1
