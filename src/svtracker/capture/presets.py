...
_PRESET_1080P: dict = {
    ...
    # PP表示「PP n /m」の数字部分だけを狙う(緑の「PP」文字はOCRを乱すので外す)。
    # 実対戦動画で数字「n /m」の位置を実測。数字の下のPPピップ(丸)は含めない。
    "self_pp": [1732, 635, 118, 46],
    "opponent_pp": [1732, 293, 118, 46],
    ...
    # プレイ表示の上部に出る「カード名バナー」。ここをOCRしてDBの名前と照合する
    # (pHashの絵柄照合より確実)。バナーの縦位置は演出の進行で数十pxぶれるため、
    # 縦に余裕を持たせた帯にしておき、読み取り側が帯内をスライドしながら最良一致を探す。
    # 実対戦動画のプレイ表示(名前バナー y≈215-255)を中心に上下へ広げた範囲。
    "play_reveal_name": [770, 195, 410, 105],
