...
- **できる**: カード画像同士の照合(pHash)、手札→盤面の差分によるプレイ検出、
  ...
  であることを利用してニュートラル以外のカードが1枚でも認識できれば判定)、対戦ログのSQLite記録、
  記録に基づく簡易な相手行動予測、盤面状況からのリーサル検出・トレード提案・
...
3. **監視 タブ**: 自分/相手のクラス欄は空欄のまま「開始」して構いません。手札・盤面で
   ...
   認識したプレイ・予測・提案はログ欄にリアルタイム表示されます。「停止」で対戦記録が
   保存されます。
...
```csv
card_id,name,clan,cost,card_type,rarity,filename,base_atk,base_hp,card_set_id
10112210,ロビンフッド,エルフ,4,フォロワー,レジェンド,10112210.png,3,3,10
```
